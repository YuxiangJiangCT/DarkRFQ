...
export default function CreateRFQ({ signer, account, isCorrectNetwork }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    tx.start(['Creating RFQ...', 'Confirming...'])

    try {
      setSubmitting(true)
      const contract = getContract(signer)
      const deadline = Math.floor(Date.now() / 1000) + Number(minutes) * 60
      const txn = await contract.createRFQ(label, isBuy, BigInt(amount), deadline, revealPolicy)
      tx.advance()
      ...
      tx.complete(() => {
        addToast('success', rfqId !== null ? `RFQ #${rfqId} created successfully` : 'RFQ created successfully')
      })

      navigate(rfqId !== null ? `/rfq/${rfqId}` : '/')
    } catch (err: unknown) {
  ...
  const disabled = submitting || !isCorrectNetwork
        ...
        <div className="bg-surface/80 backdrop-blur-xl border border-border rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <button type="submit"
              ...
              disabled={disabled}>
              {submitting ? <><Spinner /> Creating...</> : !isCorrectNetwork ? 'Wrong Network' : 'Create RFQ'}
            </button>
          </form>
        </div>
      </div>
